...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createCard(userData: CardData): string {
        return `
    ...
    function fetchAndCreateCards(): void {
        fetch(apiUrl)
            ...
            .then(users => {
                const enrichedUsers = users.map((user: any) => ({
                    ...
                    aboutMe: "Hi, I'm a final year student completing a bachelor's In information Technology in QUT, with experience. We are the company behind the wildly successful DIY channel 5-Minute Crafts, the inspirational and creative channel Bright Side."
                }));
                cardsData = enrichedUsers as CardData[]; 
                if (cardsContainer) {
